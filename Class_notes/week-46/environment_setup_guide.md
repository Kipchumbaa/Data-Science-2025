...
## Important Notes
- **Instance Type**: Current EC2 instance is t3.medium (CPU-only). RAPIDS CPU version is installed and functional for the project.
- **GPU Upgrade**: To enable full GPU acceleration, request vCPU limit increase from AWS for GPU instances (g4dn.xlarge recommended).
...
### Core Resources
- **EC2 Instance**: `i-0a444afe43fd747c7` (54.197.52.129) - Primary compute environment (t3.medium)
...
ssh -i fake-news-key.pem ec2-user@54.197.52.129
```
...
- Enables full cuML GPU acceleration for sklearn vs cuML comparisons

### Current Status
- **Issue**: New AWS accounts have 0 vCPU limit for GPU instances by default
- **Solution**: Request limit increase from AWS (takes 24-48 hours approval)
- **Alternative**: Project works with CPU RAPIDS (slower but functional)

### Request GPU Instance Limits
1. **Go to AWS Console** → EC2 → Limits
2. **Find GPU instances** (P3, G4dn, etc.)
3. **Request limit increase**:
   - Instance Type: g4dn.xlarge
   - New limit: 4 vCPUs (or more as needed)
   - Use case: Machine Learning development
4. **Wait for approval** (usually 24-48 hours)

### Upgrade Steps (After Limit Approval)
```bash
# Stop instance
...
python -c "import cudf; print('GPU RAPIDS ready')"
```

### GPU Instance Specifications
- **g4dn.xlarge**: 4 vCPUs, 16GB RAM, 1 NVIDIA T4 GPU
- **Cost**: ~$0.71/hour (vs $0.042 for t3.medium)
- **Performance**: 10-50x speedup for ML workloads
